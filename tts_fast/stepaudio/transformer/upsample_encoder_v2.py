...
from stepaudio.transformer.encoder_layer import ConformerEncoderLayer
from stepaudio.transformer.positionwise_feed_forward import PositionwiseFeedForward
from stepaudio.utils.class_utils import (
    COSYVOICE_EMB_CLASSES,
    COSYVOICE_SUBSAMPLE_CLASSES,
    COSYVOICE_ATTENTION_CLASSES,
    COSYVOICE_ACTIVATION_CLASSES,
)
from stepaudio.utils.mask import make_pad_mask
...
class UpsampleConformerEncoderV2(torch.nn.Module):
    ...
    def _init_cuda_graph(self):
        ...
        for l in range(100, 1500, 10):
            static_x = torch.zeros(
                (1, l, 512), dtype=torch.float32, device=torch.device("cuda")
            )
            static_mask = torch.ones(
                (1, 1, l), dtype=torch.bool, device=torch.device("cuda")
            )
            static_pos_emb = torch.zeros(
                (1, 2 * l - 1, 512), dtype=torch.float32, device=torch.device("cuda")
            )
            ...
            with torch.no_grad():
                with torch.cuda.graph(graph):
                    static_out_x = self._forward_impl_encoder(
                        static_inputs[0], static_inputs[1], static_inputs[2]
                    )
    ...
    def forward(
        ...
        if self.enable_cuda_graph and xs.shape[1] in self.graph_encoder:
            ...
            self.inference_buffers_encoder[xs.shape[1]]["static_inputs"][2].copy_(
                pos_emb
            )
        ...
        if self.enable_cuda_graph and xs.shape[1] in self.graph_up_encoder:
            ...
            self.inference_buffers_up_encoder[xs.shape[1]]["static_inputs"][1].copy_(
                masks
            )
            self.inference_buffers_up_encoder[xs.shape[1]]["static_inputs"][2].copy_(
                pos_emb
            )
    ...
    def forward_chunk(
        ...
        att_cache1 = (
            att_cache[: len(self.encoders), :, :, :offset1]
            if att_cache is not None
            else [None] * len(self.encoders)
        )
        att_cache2 = (
            att_cache[len(self.encoders) :]
            if att_cache is not None
            else [None] * len(self.encoders)
        )
        ...
        xs, new_cnn_cache1 = self.pre_lookahead_layer.forward_chunk(
            xs, cache=cnn_cache1
        )
        ...
        for idx, layer in enumerate(self.encoders):
            ...
            xs, _, this_new_att_cache1, _ = layer(
                xs, chunk_masks, pos_emb, att_cache=att_cache1[idx]
            )
        ...
        pos_emb = self.embed.position_encoding(
            offset=None, size=offset1 * self.up_layer.stride + xs.shape[1]
        )
        ...
        for idx, layer in enumerate(self.up_encoders):
            xs, _, this_new_att_cache2, _ = layer(
                xs, chunk_masks, pos_emb, att_cache=att_cache2[idx]
            )
        ...
        new_att_cache = torch.cat(
            [new_att_cache1.repeat(1, 1, 1, 2, 1), new_att_cache2], dim=0
        )
